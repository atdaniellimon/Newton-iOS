...
public final class DeviceBridgeService: NSObject, CLLocationManagerDelegate {
    ...
    // Persistent fallback storage for reminders and events
    ...
    public func getCurrentLocation() async -> String {
        // 1. Try fast IP-based geo-location first (reliable and instant without blocking modal prompts)
        if let ipGeo = await fetchIPLocation() {
            return ipGeo
        }
        
        // 2. CoreLocation fallback
        #if os(iOS)
