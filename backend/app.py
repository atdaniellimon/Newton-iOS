...
Proxy for OpenAI-compatible API calls + Orbit execution + File management + Code execution
...
class OrbitManager:
    ...
    def get_tools_definitions(self) -> List[Dict]:
        """Genera definiciones de herramientas para OpenAI/OpenRouter"""
        tools = []
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _do_request(self, req, timeout=600):
    ...
    def _handle_chat(self):
        ...
        custom_base_url = data.get('baseUrl')
        custom_headers = data.get('headers') or {}
        custom_body = data.get('bodyData')
        ...
        # If OpenRouter is used without key
        if 'openrouter.ai' in endpoint and not api_key:
            openrouter_api_key = os.getenv('OPENROUTER_API_KEY')
            if openrouter_api_key:
                api_key = openrouter_api_key
            else:
                self._send_json(500, {'error': 'OpenRouter API key not configured in .env or settings'})
                return
        
        model = data.get('model', 'anthropic/claude-fable-5')
        ...
        # Tools are enabled only for standard OpenAI / OpenRouter endpoints when tool definitions exist
        if tools and ('openrouter.ai' in endpoint or 'api.openai.com' in endpoint):
            payload['tools'] = tools
            payload['tool_choice'] = 'auto'
        
        # Función para hacer una llamada al endpoint LLM
        def make_llm_request(target_url, request_data):
            payload_bytes = json.dumps(request_data).encode('utf-8')
            req_headers = {
                'Content-Type': 'application/json',
                'User-Agent': 'Newton/1.0'
            }
            if api_key:
                if 'anthropic' in provider or 'messages' in target_url:
            ...
            if 'openrouter.ai' in target_url:
                req_headers['HTTP-Referer'] = os.getenv('OPENROUTER_REFERER', 'http://localhost:3000')
                req_headers['X-Title'] = os.getenv('OPENROUTER_TITLE', 'Newton')

            if custom_headers and isinstance(custom_headers, dict):
                for k, v in custom_headers.items():
                    if v: req_headers[k] = v

            req = urllib.request.Request(target_url, data=payload_bytes, headers=req_headers, method='POST')
            try:
                return self._do_request(req, timeout=600)
            except urllib.error.HTTPError as e:
                # Handle 404 fallback for local servers: /v1/chat/completions <-> /chat/completions
                if e.code == 404 and ('localhost' in target_url or '127.0.0.1' in target_url):
                    alt_url = None
                    if '/v1/chat/completions' in target_url:
                        alt_url = target_url.replace('/v1/chat/completions', '/chat/completions')
                    elif '/chat/completions' in target_url and '/v1/' not in target_url:
                        alt_url = target_url.replace('/chat/completions', '/v1/chat/completions')
                    if alt_url and alt_url != target_url:
                        req_alt = urllib.request.Request(alt_url, data=payload_bytes, headers=req_headers, method='POST')
                        return self._do_request(req_alt, timeout=600)
                raise
        
        try:
            # Primera llamada
            with make_llm_request(endpoint, payload) as response:
                response_data = json.loads(response.read().decode('utf-8'))
            
            # Anthropic Messages response check
            if 'content' in response_data and isinstance(response_data['content'], list) and 'choices' not in response_data:
                self._send_json(200, response_data)
                return

            # Verificar si la IA quiere llamar herramientas (OpenAI / OpenRouter format)
            ...
            if finish_reason == 'tool_calls' and tools:
                ...
                if tool_calls:
                    ...
                    with make_llm_request(endpoint, payload_final) as response_final:
                        final_data = json.loads(response_final.read().decode('utf-8'))
                    
                    self._send_json(200, final_data)
                    return
            ...
            self._send_json(502, {
                ...
                'detail': f'Could not reach endpoint {endpoint}: {reason}'
            })
        except Exception as e:
            self._send_json(500, {
...
def run_server(port=BACKEND_PORT, verbose=False):
    ...
    print(f'   Endpoints:')
    print(f'   POST /api/chat     - Proxy to OpenRouter (with Tool Calling)')
